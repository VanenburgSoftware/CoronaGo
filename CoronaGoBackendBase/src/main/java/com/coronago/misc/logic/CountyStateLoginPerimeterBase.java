...
public abstract class CountyStateLoginPerimeterBase<CountyStateLogin extends CountyStateLoginBase> implements IPerimeterManager<CountyStateLogin> {
	
	private AppUserPrivilegeCache userCache = CacheManager.getInstance().getCache(IAppUserPrivilegeCache.NAME);
	@Override
	public boolean canCreate(CountyStateLogin model) {
	...
	@Override
	public boolean canUpdate(CountyStateLogin model) {
		AppUserPrivilegesBase userBase = (AppUserPrivilegesBase) userCache.getCurrentUser();
		if (userBase.isDevadmin()) { return true; }
	...
	@Override
	public boolean canDelete(CountyStateLogin model) {
		AppUserPrivilegesBase userBase = (AppUserPrivilegesBase) userCache.getCurrentUser();
		if (userBase.isDevadmin()) { return true; }
	...
	@Override
	public boolean canRead(CountyStateLogin model) {
		AppUserPrivilegesBase userBase = (AppUserPrivilegesBase) userCache.getCurrentUser();
		if (userBase.isDevadmin()) { return true; }
...
protected boolean checkPatPerimeter(CountyStateLogin model, AppUserPrivilegesBase userBase) {
	return false;
	}


protected boolean checkDocPerimeter(CountyStateLogin model, AppUserPrivilegesBase userBase) {
	return false;
	}


protected boolean checkHaPerimeter(CountyStateLogin model, AppUserPrivilegesBase userBase) {
	return false;
	}


protected boolean checkAdminPerimeter(CountyStateLogin model, AppUserPrivilegesBase userBase) {
	return false;
	}


protected boolean checkSclPerimeter(CountyStateLogin model, AppUserPrivilegesBase userBase) {
	return false;
	}
	...
	private void setReadFields(AppUserPrivilegesBase userBase, AllowedFields allowedFields) {
		...
		allowedAccessFields.addAll(getTechnicalFields());
				if (userBase.isPat()) {
			String[] readFields = new String[] {"email","password","county"};
