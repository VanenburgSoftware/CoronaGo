...
import  SecurityJson  from '@app/security/corona-tracker/corona-tracker-detail/corona-tracker-detail.security';
import { CoronaTrackerDetailHandler } from '@app/custom/corona-tracker/corona-tracker-detail/corona-tracker-detail.handler';
...
@Component({
  ...
  templateUrl: './corona-tracker-detail.component.html',
  styleUrls: ['./corona-tracker-detail.component.scss']
})
export class CoronaTrackerDetailComponent extends CoronaTrackerDetailHandler implements OnInit {
...
this.local.formConfig.coronaTracker         
  ...
  public CoronaTrackerDetailFormInitComplete(form: any) {
  ...
  private getData() {
    ...
    const params = {
      ...
      this.coronaTrackerService.getCoronaTrackerBySid(params).subscribe((response: any) => {
  ...
  private onCreateAction(btn: any): void {
    ...
    confirm.subscribe((isConfirmed: boolean) => {
    	if (isConfirmed) {
    		const createSubscriber = this.coronaTrackerService.createCoronaTracker(this.id);
  ...
  private onUpdateAction(btn: any): void {
    ...
    confirm.subscribe((isConfirmed: boolean) => {
    	if (isConfirmed) {
    		const updateSubscriber = this.coronaTrackerService.updateCoronaTracker(this.id);
  ...
  private onDeleteAction(btn: any): void {
    ...
    confirm.subscribe((isConfirmed: boolean) => {
    	if (isConfirmed) {
    		const deleteSubscriber = this.coronaTrackerService.deleteCoronaTracker(this.id);
