...
import  SecurityJson  from '@app/security/heat-map/heat-map-detail/heat-map-detail.security';
import { HeatMapDetailHandler } from '@app/custom/heat-map/heat-map-detail/heat-map-detail.handler';
...
export class HeatMapDetailComponent extends HeatMapDetailHandler implements OnInit {
...
this.local.formConfig.heatMap         
  ...
  public HeatMapDetailFormInitComplete(form: any) {
  ...
  private getData() {
    ...
    const params = {
      ...
      this.heatMapService.getHeatMapBySid(params).subscribe((response: any) => {
  ...
  private onCreateAction(btn: any): void {
    ...
    confirm.subscribe((isConfirmed: boolean) => {
    	if (isConfirmed) {
    		const createSubscriber = this.heatMapService.createHeatMap(this.id);
  ...
  private onUpdateAction(btn: any): void {
    ...
    confirm.subscribe((isConfirmed: boolean) => {
    	if (isConfirmed) {
    		const updateSubscriber = this.heatMapService.updateHeatMap(this.id);
  ...
  private onDeleteAction(btn: any): void {
    ...
    confirm.subscribe((isConfirmed: boolean) => {
    	if (isConfirmed) {
    		const deleteSubscriber = this.heatMapService.deleteHeatMap(this.id);
