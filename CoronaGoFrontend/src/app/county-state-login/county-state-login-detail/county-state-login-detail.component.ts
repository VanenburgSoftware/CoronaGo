...
import  SecurityJson  from '@app/security/county-state-login/county-state-login-detail/county-state-login-detail.security';
import { CountyStateLoginDetailHandler } from '@app/custom/county-state-login/county-state-login-detail/county-state-login-detail.handler';
import { CountyStateLoginService } from '../county-state-login.service';

import { ModalPopupService } from '@app/shared/services/modal-popup.service';

@Component({
  selector: 'app-county-state-login-detail',
  templateUrl: './county-state-login-detail.component.html',
  styleUrls: ['./county-state-login-detail.component.scss']
})
export class CountyStateLoginDetailComponent extends CountyStateLoginDetailHandler implements OnInit {
  ...
  constructor(
    ...
    private countyStateLoginService: CountyStateLoginService,
...
this.local.formConfig.countyStateLogin         
  ...
  public CountyStateLoginDetailFormInitComplete(form: any) {
  ...
  private getData() {
    ...
    const params = {
      ...
      this.countyStateLoginService.getCountyStateLoginBySid(params).subscribe((response: any) => {
  ...
  private onCreateAction(btn: any): void {
    ...
    confirm.subscribe((isConfirmed: boolean) => {
    	if (isConfirmed) {
    		const createSubscriber = this.countyStateLoginService.createCountyStateLogin(this.id);
  ...
  private onUpdateAction(btn: any): void {
    ...
    confirm.subscribe((isConfirmed: boolean) => {
    	if (isConfirmed) {
    		const updateSubscriber = this.countyStateLoginService.updateCountyStateLogin(this.id);
  ...
  private onDeleteAction(btn: any): void {
    ...
    confirm.subscribe((isConfirmed: boolean) => {
    	if (isConfirmed) {
    		const deleteSubscriber = this.countyStateLoginService.deleteCountyStateLogin(this.id);
