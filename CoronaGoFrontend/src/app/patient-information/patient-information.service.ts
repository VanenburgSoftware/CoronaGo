...
export class PatientInformationService {
   ...
   getPrototypingData(columns: any): any {
	...
	const possibleValues = {
		...
		insuranceType : ['0-3 days','3-7 days','>7 days'],
		status : ['Risk score to be calculated','Risk self accessed','Consultation required','Consultation not required','Consultation scheduled','Consultation completed (Test prescribed)','Consultation completed (Weekly monitoring']
	};
  ...
  public updatePatientInformation(params: any): any {
    const serviceOpts=ApiConstants.updatePatientInformation;
  ...
  public getPatientInformationBySid(params: any): any {
    const serviceOpts=ApiConstants.getPatientInformationBySid;
    const subject = new Observable(observer => {
      this.baseService[ serviceOpts.method.toLowerCase() ](serviceOpts,params).subscribe((response) => {
        observer.next(response);
      },
  ...
  public deletePatientInformation(params: any): any {
    const serviceOpts=ApiConstants.deletePatientInformation;
    const subject = new Observable(observer => {
      this.baseService[ serviceOpts.method.toLowerCase() ](serviceOpts,params).subscribe((response) => {
        observer.next(response);
      },
  ...
  public createPatientInformation(params: any): any {
    const serviceOpts=ApiConstants.createPatientInformation;
    const subject = new Observable(observer => {
      this.baseService[ serviceOpts.method.toLowerCase() ](serviceOpts,params).subscribe((response) => {
        observer.next(response);
      },
