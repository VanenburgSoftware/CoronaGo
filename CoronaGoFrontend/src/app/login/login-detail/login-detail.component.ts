...
import  SecurityJson  from '@app/security/login/login-detail/login-detail.security';
import { LoginDetailHandler } from '@app/custom/login/login-detail/login-detail.handler';
...
export class LoginDetailComponent extends LoginDetailHandler implements OnInit {
  ...
  private getFormConfig(securityJson?: any) {
    this.local.formConfig = {};
   	...
   	this.local.formConfig.loginPage = {

      security_code: 'loginPage',
      label: 'Login Page',
      type: 'customelement',
      name: 'loginPage',
      placeholder: '',
    ...
    let config= {
      initCallback: this.LoginDetailFormInitComplete.bind(this),
      collapsible: false,
...
this.local.formConfig.loginPage         
  ...
  public LoginDetailFormInitComplete(form: any) {
  ...
  private getData() {
    ...
    const params = {
      ...
      this.loginService.getLoginBySid(params).subscribe((response: any) => {
  ...
  private saveDataThenFiles(method, splittedData, saveFiles?: boolean) {
    let service;
	if (method === 'post') {
		service = this.loginService.createLogin(splittedData.data);

	} else if (method === 'put') {
		if (!splittedData.data.sid) {
			splittedData.data.sid = this.id;
		}
		service = this.loginService.updateLogin(splittedData.data);
  ...
  private onCreateAction(btn: any): void {
    ...
    confirm.subscribe((isConfirmed: boolean) => {
    	if (isConfirmed) {
    		const createSubscriber = this.loginService.createLogin(this.id);
  ...
  private onUpdateAction(btn: any): void {
    ...
    confirm.subscribe((isConfirmed: boolean) => {
    	if (isConfirmed) {
    		const updateSubscriber = this.loginService.updateLogin(this.id);
  ...
  private onDeleteAction(btn: any): void {
    ...
    confirm.subscribe((isConfirmed: boolean) => {
    	if (isConfirmed) {
    		const deleteSubscriber = this.loginService.deleteLogin(this.id);
