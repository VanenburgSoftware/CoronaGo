...
import  SecurityJson  from '@app/security/stock-level-by-hospital/stock-level-by-hospital-detail/stock-level-by-hospital-detail.security';
import { StockLevelByHospitalDetailHandler } from '@app/custom/stock-level-by-hospital/stock-level-by-hospital-detail/stock-level-by-hospital-detail.handler';
import { StockLevelByHospitalService } from '../stock-level-by-hospital.service';

import { ModalPopupService } from '@app/shared/services/modal-popup.service';

@Component({
  selector: 'app-stock-level-by-hospital-detail',
  templateUrl: './stock-level-by-hospital-detail.component.html',
  styleUrls: ['./stock-level-by-hospital-detail.component.scss']
})
export class StockLevelByHospitalDetailComponent extends StockLevelByHospitalDetailHandler implements OnInit {
...
this.local.formConfig.stockLevelByHospital         
  ...
  public StockLevelByHospitalDetailFormInitComplete(form: any) {
  ...
  private getData() {
    ...
    const params = {
      ...
      this.stockLevelByHospitalService.getStockLevelByHospitalBySid(params).subscribe((response: any) => {
  ...
  private onCreateAction(btn: any): void {
    ...
    confirm.subscribe((isConfirmed: boolean) => {
    	if (isConfirmed) {
    		const createSubscriber = this.stockLevelByHospitalService.createStockLevelByHospital(this.id);
  ...
  private onUpdateAction(btn: any): void {
    ...
    confirm.subscribe((isConfirmed: boolean) => {
    	if (isConfirmed) {
    		const updateSubscriber = this.stockLevelByHospitalService.updateStockLevelByHospital(this.id);
  ...
  private onDeleteAction(btn: any): void {
    ...
    confirm.subscribe((isConfirmed: boolean) => {
    	if (isConfirmed) {
    		const deleteSubscriber = this.stockLevelByHospitalService.deleteStockLevelByHospital(this.id);
