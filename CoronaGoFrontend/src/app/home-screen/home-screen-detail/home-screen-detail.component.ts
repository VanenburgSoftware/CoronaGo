...
import  SecurityJson  from '@app/security/home-screen/home-screen-detail/home-screen-detail.security';
import { HomeScreenDetailHandler } from '@app/custom/home-screen/home-screen-detail/home-screen-detail.handler';
...
@Component({
  ...
  templateUrl: './home-screen-detail.component.html',
  styleUrls: ['./home-screen-detail.component.scss']
})
export class HomeScreenDetailComponent extends HomeScreenDetailHandler implements OnInit {
...
this.local.formConfig.homeScreen         
  ...
  public HomeScreenDetailFormInitComplete(form: any) {
  ...
  private getData() {
    ...
    const params = {
      ...
      this.homeScreenService.getHomeScreenBySid(params).subscribe((response: any) => {
  ...
  private onCreateAction(btn: any): void {
    ...
    confirm.subscribe((isConfirmed: boolean) => {
    	if (isConfirmed) {
    		const createSubscriber = this.homeScreenService.createHomeScreen(this.id);
  ...
  private onUpdateAction(btn: any): void {
    ...
    confirm.subscribe((isConfirmed: boolean) => {
    	if (isConfirmed) {
    		const updateSubscriber = this.homeScreenService.updateHomeScreen(this.id);
  ...
  private onDeleteAction(btn: any): void {
    ...
    confirm.subscribe((isConfirmed: boolean) => {
    	if (isConfirmed) {
    		const deleteSubscriber = this.homeScreenService.deleteHomeScreen(this.id);
