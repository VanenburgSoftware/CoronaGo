...
import { AllPatientsNotScheduledListHandler } from '@custom/patient-questionaire/all-patients-not-scheduled-list/all-patients-not-scheduled-list.handler';
import { PatientQuestionaireService } from '../patient-questionaire.service';
import { AllPatientsNotScheduledListMobileViewComponent } from './all-patients-not-scheduled-list-mobile-view/all-patients-not-scheduled-list-mobile-view.component';

@Component({
  selector: 'app-all-patients-not-scheduled-list',
  templateUrl: './all-patients-not-scheduled-list.component.html',
  styleUrls: ['./all-patients-not-scheduled-list.component.scss']
})
export class AllPatientsNotScheduledListComponent extends AllPatientsNotScheduledListHandler implements OnInit, AfterViewInit {
  ...
  public pageName = 'ALL_PATIENTS_NOT_SCHEDULED_LIST';
  ...
  private loadGridConfig(): any {
    ...
    const gridConfig: any = {
      ...
      mobileTemplate: AllPatientsNotScheduledListMobileViewComponent,
    ...
    if (this.isPrototyping) {
      ...
      gridConfig.ajaxUrl = '/' + AppConstants.apihost + '/' + ApiConstants.getPatientQuestionaireAll_AllPatientsNotScheduled.url;
    }
  ...
  loadKanbanOptions() {
    const config = {
      url: this.isPrototyping ? '' : '/' + AppConstants.apihost + '/' + ApiConstants.getPatientQuestionaireAll_AllPatientsNotScheduled.url,
      template: '',
      lanes: [],
      groupBy: '',
      laneBy: '',
