...
import { MyPatientsNotScheduledListHandler } from '@custom/patient-questionaire/my-patients-not-scheduled-list/my-patients-not-scheduled-list.handler';
import { PatientQuestionaireService } from '../patient-questionaire.service';
import { MyPatientsNotScheduledListMobileViewComponent } from './my-patients-not-scheduled-list-mobile-view/my-patients-not-scheduled-list-mobile-view.component';

@Component({
  selector: 'app-my-patients-not-scheduled-list',
  templateUrl: './my-patients-not-scheduled-list.component.html',
  styleUrls: [ './my-patients-not-scheduled-list.component.scss' ]
})
export class MyPatientsNotScheduledListComponent extends MyPatientsNotScheduledListHandler implements OnInit, AfterViewInit {
  ...
  public pageName = 'MY_PATIENTS_NOT_SCHEDULED_LIST';
  ...
  private loadGridConfig(): any {
    ...
    const gridConfig: any = {
      ...
      mobileTemplate: MyPatientsNotScheduledListMobileViewComponent,
    ...
    if (this.isPrototyping) {
      ...
      gridConfig.ajaxUrl = '/' + AppConstants.apihost + '/' + ApiConstants.getPatientQuestionaireAll_MyPatientsNotScheduled.url;
    }
  ...
  loadKanbanOptions() {
    const config = {
      url: this.isPrototyping ? '' : '/' + AppConstants.apihost + '/' + ApiConstants.getPatientQuestionaireAll_MyPatientsNotScheduled.url,
      template: '',
      lanes: [],
      groupBy: '',
      laneBy: '',
