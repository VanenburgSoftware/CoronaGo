...
import { AllPatientsScheduledListHandler } from '@custom/patient-questionaire/all-patients-scheduled-list/all-patients-scheduled-list.handler';
import { PatientQuestionaireService } from '../patient-questionaire.service';
import { AllPatientsScheduledListMobileViewComponent } from './all-patients-scheduled-list-mobile-view/all-patients-scheduled-list-mobile-view.component';

@Component({
  selector: 'app-all-patients-scheduled-list',
  templateUrl: './all-patients-scheduled-list.component.html',
  styleUrls: ['./all-patients-scheduled-list.component.scss']
})
export class AllPatientsScheduledListComponent extends AllPatientsScheduledListHandler implements OnInit, AfterViewInit {
  ...
  public pageName = 'ALL_PATIENTS_SCHEDULED_LIST';
  ...
  private loadGridConfig(): any {
    ...
    const gridConfig: any = {
      ...
      mobileTemplate: AllPatientsScheduledListMobileViewComponent,
    ...
    if (this.isPrototyping) {
      ...
      gridConfig.ajaxUrl = '/' + AppConstants.apihost + '/' + ApiConstants.getPatientQuestionaireAll_AllPatientsScheduled.url;
    }
  ...
  private onNeedsConsultationAction(selectedRowIds: string, config: any, source?: string): void {
    ...
    const NeedsConsultationSubscriber = this.patientQuestionaireService.allPatientsScheduledRowNeedsConsultation(params);
  ...
  loadKanbanOptions() {
    const config = {
      url: this.isPrototyping ? '' : '/' + AppConstants.apihost + '/' + ApiConstants.getPatientQuestionaireAll_AllPatientsScheduled.url,
      template: '',
      lanes: [],
      groupBy: '',
      laneBy: '',
